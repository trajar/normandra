...
abstract public class SaveTest extends BaseTest {
    ...
    public void testEmbeddableField() throws Exception {
        ...
        session.save(new StoreEntity("Store A", address));
        session.save(new StoreEntity("Store B", null));

        session.clear();

        final StoreEntity storeA = session.get(StoreEntity.class, "Store A");
        Assert.assertNotNull(storeA);
        Assert.assertNotNull(storeA.address);
        Assert.assertEquals("nowhereville", storeA.address.city);

        final StoreEntity storeB = session.get(StoreEntity.class, "Store B");
    ...
    public void testJoinColumn() throws Exception {
        ...
        student = session.get(StudentEntity.class, student.getId());
        ...
        session.save(new StudentEntity("bob", classroom));
        session.save(new StudentEntity("jane", classroom));
        session.clear();

        final ClassEntity associated = session.get(ClassEntity.class, classroom.getId());
    ...
    public void testMultiJoin() throws Exception {
        ...
        ZooEntity existing = session.get(ZooEntity.class, zoo.getId());
    ...
    public void testComposite() throws Exception {
        ...
        session.save(student);
        Assert.assertNull(session.get(StudentIndexEntity.class, "fred"));
        Assert.assertNull(session.get(StudentIndexEntity.class, "101"));
